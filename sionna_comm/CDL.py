...
from sionna.phy.channel.tr38901 import Topology, ChannelCoefficientsGenerator
from sionna.phy.channel.tr38901 import Rays
from sionna.phy.channel.tr38901 import models # pylint: disable=relative-beyond-top-level
...
class CDL(ChannelModel):
    ...
    direction : "uplink" | "downlink" | "sidelink"
        Link direction

    ut_orientation : `None` (default) | [3], `tf.float`
        Orientation of the UT. If set to `None`, [:math:`\pi`, 0, 0] is used.

    bs_orientation : `None` (default) | [3], `tf.float`
    ...
    def __init__(   self,
        ...
        assert direction in('uplink', 'downlink', "sidelink"), "Invalid link direction"
        ...
        if self._direction == 'downlink':
            self._moving_end = 'rx'
            self._tx_array = rx_array
            self._rx_array = tx_array
        ...
        assert model in ("A", "B", "C", "D", "E","urban_los","urban_nlos","urban_nlosv","highway_los"), "Invalid CDL model"
        if model == 'A':
            parameters_fname = "json/CDL-A.json"
        elif model == 'B':
            parameters_fname = "json/CDL-B.json"
        elif model == 'C':
            parameters_fname = "json/CDL-C.json"
        elif model == 'D':
            parameters_fname = "json/CDL-D.json"
        elif model == 'E':
            parameters_fname = "json/CDL-E.json"
        elif model == "urban_los":
            parameters_fname = "json/urban_los.json"        
        elif model == "urban_nlos":
            parameters_fname = "json/urban_nlos.json"        
        elif model == "urban_nlosv":
            parameters_fname = "json/urban_nlosv.json"
        elif model == "highway_los":
            parameters_fname = "json/highway_los.json"
    ...
    def _load_parameters(self, fname):
        ...
        * xpr : XPR in dB
        ...
        invariable_path = Path("sionna_comm")
        source = invariable_path / f"{fname}"
